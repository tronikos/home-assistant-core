...
from homeassistant.components.nest_legacy import _PLATFORMS
from homeassistant.components.nest_legacy.const import (
    CONF_ACCOUNT_TYPE,
    CONF_COOKIES,
    CONF_ISSUE_TOKEN,
    DOMAIN,
...
def mock_nest_client(
    ...
    with patch(
        "homeassistant.components.nest_legacy.coordinator.NestClient", autospec=True
    ) as client_class:
...
def patch_platforms(platforms: list[Platform]) -> Generator[None]:
    ...
    with patch("homeassistant.components.nest_legacy._PLATFORMS", platforms):
        yield
...
def mock_setup_entry() -> Generator[AsyncMock]:
    ...
    with patch(
        "homeassistant.components.nest_legacy.async_setup_entry", return_value=True
    ) as mock:
        yield mock
